...
#define DESCRIPTION "Module Ecriture/lecture 3 periph,buffer variable"
...
static ssize_t buffer_read_dest(struct file *f, char *buff, size_t size, loff_t *offp);
...
static int buffer_open(struct inode *in, struct file *f){
    ...
    else if(iminor(in) == 1){//Cas ou on read avec destruction
        my_fops.read = &buffer_read_dest;
    }
    else if(iminor(in) == 2){//cas du write
...
static ssize_t buffer_read(struct file *f, char *buff, size_t size, loff_t *offp){
	...
	//printk(KERN_ALERT " Taille dataSizeTmp= %d",dataSizeTmp);
    
    
	if(dataSizeTmp > 0) {
            //tester first entry
            if(dataSizeTmp == dataSize){
                elementNonRead = list_first_entry(&contenu, BUFFER, liste);
            }
            else{
                elementNonRead = list_next_entry(elementNonRead,liste);
            }
            
            sizeToCopy = MIN (elementNonRead->bufSize, size);
            if(copy_to_user(buff, elementNonRead->bufData, sizeToCopy)==0){
                dataSizeTmp = dataSizeTmp - sizeToCopy;
            }
            else{
                printk(KERN_ALERT "ERROR : copy_to_user.\n");
                return -EFAULT;
            }
            //printk(KERN_ALERT "taille a copier:%d\n",sizeToCopy);
